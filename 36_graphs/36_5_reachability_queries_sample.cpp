...
void dfs(const std::vector<std::vector<int> >& graph, int cc_id, int node,
         std::set<int>& visited, std::map<int, int>& node_to_cc) {
    visited.insert(node);
...
std::vector<bool> reachability_queries(const std::vector<std::vector<int> >& graph,
